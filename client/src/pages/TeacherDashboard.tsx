...
type ClassScoreSummary = { className: string; average: number; attempts: number };

function classChartSvg(data: ClassScoreSummary[]) {
  const width = 1100;
  const height = 620;
  const chartTop = 150;
  const chartHeight = 330;
  const colors = ["#c7ff35", "#bca8ff", "#ffb870"];
  const bars = data.map((item, index) => {
    const x = 170 + index * 300;
    const barHeight = Math.max(4, Math.round((item.average / 100) * chartHeight));
    const y = chartTop + chartHeight - barHeight;
    return `<rect x="${x}" y="${y}" width="150" height="${barHeight}" rx="8" fill="${colors[index % colors.length]}"/><text x="${x + 75}" y="${y - 18}" text-anchor="middle" fill="#f1f4ed" font-size="25" font-family="Arial" font-weight="700">${item.average}/100</text><text x="${x + 75}" y="${chartTop + chartHeight + 38}" text-anchor="middle" fill="#d8ddd4" font-size="20" font-family="Arial">${item.className}</text><text x="${x + 75}" y="${chartTop + chartHeight + 66}" text-anchor="middle" fill="#87918d" font-size="15" font-family="Arial">${item.attempts} percobaan</text>`;
  }).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#11141b"/><text x="55" y="58" fill="#c7ff35" font-size="16" font-family="Arial" letter-spacing="3">MARKUPLAB · LAPORAN BULANAN GURU</text><text x="55" y="105" fill="#f1f4ed" font-size="32" font-family="Arial" font-weight="700">Perbandingan Nilai Rata-rata Antar Kelas</text><line x1="130" y1="${chartTop + chartHeight}" x2="1010" y2="${chartTop + chartHeight}" stroke="#39414b" stroke-width="2"/><line x1="130" y1="${chartTop}" x2="130" y2="${chartTop + chartHeight}" stroke="#39414b" stroke-width="2"/><text x="110" y="${chartTop + 8}" text-anchor="end" fill="#87918d" font-size="14" font-family="Arial">100</text><text x="110" y="${chartTop + chartHeight / 2 + 5}" text-anchor="end" fill="#87918d" font-size="14" font-family="Arial">50</text><text x="110" y="${chartTop + chartHeight + 5}" text-anchor="end" fill="#87918d" font-size="14" font-family="Arial">0</text>${bars}<text x="55" y="580" fill="#87918d" font-size="14" font-family="Arial">Dibuat: ${new Date().toLocaleDateString("id-ID")}</text></svg>`;
}

function downloadClassChartPng(data: ClassScoreSummary[]) {
  const svg = classChartSvg(data);
  ...
  image.onload = () => {
    ...
    anchor.download = `Perbandingan_Nilai_Antar_Kelas_${new Date().toISOString().slice(0, 10)}.png`;
    anchor.click();
  };
  image.src = url;
}

function downloadClassChartPdf(data: ClassScoreSummary[]) {
  const lines = ["MARKUPLAB — LAPORAN PERBANDINGAN KELAS", `Diekspor: ${new Date().toLocaleString("id-ID")}`, "", "Kelas | Nilai rata-rata | Jumlah percobaan", ...data.map((item) => `${item.className} | ${item.average}/100 | ${item.attempts} percobaan`), "", "Catatan: Nilai rata-rata dihitung dari seluruh percobaan kuis yang tersimpan.", "", "Tanda tangan guru,", "", "____________________________", "Nama guru: __________________"];
  const content = ["BT", "/F1 10 Tf", "40 800 Td", ...lines.flatMap((line) => [`(${pdfText(line)}) Tj`, "0 -17 Td"]), "ET"].join("\n");
  const objects = ["<< /Type /Catalog /Pages 2 0 R >>", "<< /Type /Pages /Kids [3 0 R] /Count 1 >>", "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>", "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>", `<< /Length ${content.length} >>\nstream\n${content}\nendstream`];
  let pdf = "%PDF-1.4\n";
  const offsets: number[] = [];
  objects.forEach((object, index) => { offsets.push(pdf.length); pdf += `${index + 1} 0 obj\n${object}\nendobj\n`; });
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `Perbandingan_Nilai_Antar_Kelas_${new Date().toISOString().slice(0, 10)}.pdf`;
  anchor.click();
  URL.revokeObjectURL(url);
}
